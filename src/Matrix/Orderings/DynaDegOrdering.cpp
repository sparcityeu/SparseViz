    ...
    {//that many iterations
        ...
        { //the chosen one is row
            ...
            for(int p = row_ptrs[vtx]; p < row_ptrs[vtx+1]; p++)
            {
                ...
                if(nbrdeg != -1)
                { //this column has not been permuted before so decrement its degree
                    //remove it from its vector
                    int last = vtxes[nbrdeg].back();
                    vtxes[nbrdeg].pop_back();

                    if(last != m + nbr)
                    { //nbr is not the last node so move the last to its location

...
{
public:
    TensorGPUKernel(const std::string& kernelName, const std::vector<int>& gridSizes, const std::vector<int>& blockSizes, const std::vector<int>& sharedMemorySizes, int nRun, int nIgnore)
    :   kernelName(kernelName), gridSizes(gridSizes), blockSizes(blockSizes), sharedMemorySizes(sharedMemorySizes), nRun(nRun), nIgnore(nIgnore) {}
    ...
    virtual void hostFunction(const SparseTensor& A, int iterNumber, int gridSize, int blockSize, int sharedMemorySize) = 0;

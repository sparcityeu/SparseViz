...
int main(int argc, char* argv[])
{
    ...
    omp_init_lock(&(TensorKPartiteOrdering::kpartite_lock));
    ...
    omp_destroy_lock(&(TensorKPartiteOrdering::kpartite_lock));

    std::unordered_map<std::string, SparseMatrix*>::iterator kpartite_matrices_iter;
    for (kpartite_matrices_iter = (TensorKPartiteOrdering::kpartite_matrices).begin(); kpartite_matrices_iter != (TensorKPartiteOrdering::kpartite_matrices).end(); kpartite_matrices_iter++) {
        delete kpartite_matrices_iter->second;
    }

    std::unordered_map<std::string, omp_lock_t*>::iterator kpartite_locks_iter;
    for (kpartite_locks_iter = (TensorKPartiteOrdering::kpartite_locks).begin(); kpartite_locks_iter != (TensorKPartiteOrdering::kpartite_locks).end(); kpartite_locks_iter++) {
